                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="d-flex justify-content-end"> <!-- Updated class name -->
                                <a href="{{ route('admin.add-category') }}" class="btn btn-success mb-3"><i class="fa fa-plus"></i> Add Category</a> <!-- Removed flex-justify-end class -->
                            </div>
                            <table table id="categoriestable" class="table table-bordered dt-responsive  nowrap w-100">
                                <thead>
                                    <tr>
                                        <th>ID</th>
                                        <th>Name</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><button type="button" id="active-inactive" class='btn btn-{{ $product->status == 1 ? "success":"danger"}} btn-rounded' data-category-id="{{ $product->id }}">{{$product->status == 1 ? "Active":"Inactive"}}</button></td>
                                        <td>{{ $product->price }}</td>
...
<script>
    var categoryTable = $('#categoriestable').DataTable({
            ...
            serverSide: false,
            ajax: {
                url: "{{ route('admin.all-categories') }}",
                dataType: 'json',
                dataSrc: 'categories' // Set the data source to an empty string to match the response structure
            },
            columns: [{
                    data: 'id'
                },
                ...
                },
                {
                    ...
                    render: function(data) {
                        ...
                        var buttonId = 'active-inactive-' + data
                            .id; // Generate unique id using category ID
                        return '<button type="button" id="' + buttonId + '" class="btn btn-' +
                            statusClass + ' btn-rounded active-inactive" data-category-id="' +
                            data.id + '">' +
                            statusText + '</button>';
                    }
                },
                {
                    data: null,
                    render: function(data) {
                        return '<a class="btn btn-success btn-sm mx-2" title="Edit" href="{{ route('admin.add-category') }}?id=' + data.id +
                            '"><i class="mdi mdi-pencil d-block font-size-16"></i></a>'
                            +'<a class="delete btn btn-danger btn-sm" title="Delete" href="{{ route('admin.delete-category') }}?id=' + data.id +
    ...
    `;
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $("#active-inactive-" + productId + "[data-category-id='" + productId + "']")
                        .text(result.value);
                        // table.draw();
                }
            }).catch(error => {
