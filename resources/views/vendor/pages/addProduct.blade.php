@include('vendor.layouts.header', ['title' => 'Dashboard'])
...
$(document).ready(function() {
    ...
    if (gallery) {
        ...
        const existingImgWrap = displayExistingImages(gallery);
        const uploadImgWrap = displayUploadedImages(fileInput);
        const showImgDiv = document.getElementById('show-img');
        showImgDiv.appendChild(existingImgWrap);
        showImgDiv.appendChild(uploadImgWrap);
    }

    // Listen for file input change event
    document.getElementById('additional_images').addEventListener('change', function() {
        const uploadImgWrap = displayUploadedImages(this);
        const showImgDiv = document.getElementById('show-img');
        showImgDiv.appendChild(uploadImgWrap);
    });

    // Function to display existing images
    function displayExistingImages(gallery) {
        const existingImgWrap = document.createElement('div');
        existingImgWrap.classList.add('upload__img-wrap');
        ...
        if (gallery && gallery.length > 0) {
            gallery.forEach((item) => {
                ...
                image.style.backgroundImage = `url({{asset('upload')}}/${item.image})`;
                ...
                closeIcon.addEventListener('click', function() {
                    ...
                    const imageId = image.dataset.number;
                    $.ajax({
                        url: "{{ route('vendor.delete-gallery', ['imageId' => '__imageId__']) }}".replace('__imageId__', imageId),
                        type: 'DELETE',
                ...
                existingImgWrap.appendChild(imageBox);
            });
        }

        return existingImgWrap;
    }

    // Function to display uploaded images
    function displayUploadedImages(fileInput) {
